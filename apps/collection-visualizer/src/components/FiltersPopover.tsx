    ...
    <div>
      <div className="mb-2 flex items-center justify-between">
        ...
        <span className="text-xs text-muted-foreground">
          {props.format(lo)} – {props.format(hi)}
        </span>
      </div>
      <Slider
        min={min}
      ...
      <Popover>
        <PopoverTrigger asChild>
          <Button variant="outline" size="sm">
            ...
            {count > 0 && (
              <span className="ml-0.5 rounded-full bg-primary px-1.5 text-xs font-bold text-primary-foreground">
                {count}
              </span>
            )}
          </Button>
            ...
            <div className="space-y-4 p-4">
              <section>
                      ...
                      <Tooltip key={m.sym}>
                        <TooltipTrigger asChild>
                          <button
                            ...
                            className={cn(
                              "h-7 w-7 rounded-full transition cursor-pointer",
                              on ? "ring-2 ring-ring ring-offset-1 ring-offset-popover" : "opacity-40 hover:opacity-90",
                            )}
                          >
                ...
                <div className="mt-2 flex gap-2">
                  <Button
                    variant={f.colorless ? "default" : "outline"}
                    size="sm"
                    onClick={() => set({ colorless: !f.colorless })}
                  >
                    Colorless
                  </Button>
                  <Button
                    variant={f.multicolor ? "default" : "outline"}
                    size="sm"
                    onClick={() => set({ multicolor: !f.multicolor })}
                  >
                    Multicolor
                  </Button>
                </div>
                  ...
                  <div className="space-y-0.5 p-2 pr-2.5">
                    {visibleSets.length === 0 ? (
                      ...
                      visibleSets.map((s) => (
                        <label
                          key={s.code}
                          className="flex min-w-0 cursor-pointer items-center gap-2 rounded px-1 py-1 hover:bg-accent"
                        >
