...
export const StackTile = memo(function StackTile(props: StackTileProps) {
  ...
  const manaCost = repTwoSided ? repFaces[flips % 2].manaCost : rep.enriched.manaCost

  const renderFace = (v: Tile, isRep: boolean) => {
    if (isRep && repTwoSided) {
      return <FlipImage front={cardImage(repFaces[0], 'normal')} back={cardImage(repFaces[1], 'normal')} rotations={flips} alt={v.name} loading="lazy" />
    }
    const src = v.enriched.imageNormal ?? v.enriched.imageSmall
    if (src) return <img src={src} alt={v.name} loading="lazy" className="absolute inset-0 h-full w-full object-contain" />
    return <div className="absolute inset-0 flex items-center justify-center p-2 text-center text-xs text-muted-foreground">{v.name}</div>
  }
  ...
  const badges = (
    <>
      {quantity > 1 && (
        <span className="absolute bottom-1 left-1/2 -translate-x-1/2 rounded bg-black/80 px-1.5 py-0.5 text-xs font-semibold text-white">×{quantity}</span>
      )}
      {rep.finish !== 'normal' && (
